...
// .wrangler/tmp/bundle-txEGvh/checked-fetch.js
...
// .wrangler/tmp/bundle-txEGvh/strip-cf-connecting-ip-header.js
...
async function verifyJWT(token, secret) {
  try {
    ...
    const { payload } = await jwtVerify(token, encoder2.encode(secret))
    return payload
  } catch (error) {
...
function createErrorResponse(message2, status = 400, headers = {}) {
...
async function generateSignedUrl(bucket, key, expiresIn = 900) {
  try {
...
async function proxyToStrapi(request, env, corsHeaders) {
...
__name(proxyToStrapi, "proxyToStrapi")

// src/handlers/remix.ts
async function handleTemplateRemix(request, env, payload) {
  const url = new URL(request.url)
...
__name(handleTemplateRemix, "handleTemplateRemix")
...
// .wrangler/tmp/bundle-txEGvh/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-txEGvh/middleware-loader.entry.ts
