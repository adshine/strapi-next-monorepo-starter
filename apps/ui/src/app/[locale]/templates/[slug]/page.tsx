        ...
        <div className="grid grid-cols-1 gap-10 lg:grid-cols-12">
          <div className="lg:col-span-7">
            <nav className="flex flex-wrap items-center gap-2 py-8 text-sm text-[var(--text-muted)]">
              ...
              <span>→</span>
              <span className="text-[var(--text-primary)]">{template.title}</span>
            </nav>
            ...
            <div className="sticky top-8 space-y-6 py-8">
              <div className="space-y-4">
                <p className="text-sm text-[var(--text-muted)]">
                  by{' '}
                  <span className="font-semibold text-[var(--text-primary)]">{template.creator}</span>{' '}
                  in{' '}
                  <span className="font-semibold text-[var(--text-primary)]">{template.category}</span>
                </p>
                <h1 className="text-4xl font-bold leading-tight md:text-5xl">{template.title}</h1>
                <p className="text-[var(--text-muted)]">{template.description}</p>
              ...
              <div className="flex flex-col gap-3 sm:flex-row">
                ...
                >
                  ...
                  {isFreePlan ? 'Get for Free' : `Unlock with ${requiredPlan?.name}`}
                </Button>
                <Button
                  variant="outline"
                  size="lg"
                ...
                >
                  <Heart className={`mr-2 h-5 w-5 ${isFavorited ? 'fill-current text-[var(--accent-success)]' : ''}`} />
              ...
              <div className="rounded-2xl border border-[var(--border-neutral)] bg-[var(--bg-elevated)] p-6">
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Badge variant="secondary" className="bg-[var(--accent-primary)]/20 text-[var(--accent-primary)]">
              ...
              <div className="flex flex-wrap gap-2 pt-2">
                {template.tags.map((tag) => (
                  <Badge key={tag} variant="secondary" className="bg-[var(--bg-elevated)] text-[var(--text-muted)]">
                    #{tag}
                  </Badge>
                ))}
              </div>
          ...
          <section className="py-16">
            ...
            <div className="mt-10 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
              {relatedTemplates.map((item) => (
                <Card
                  key={item.id}
                  className="bg-[var(--bg-elevated)]/40 border-[var(--border-neutral)] hover:border-[var(--accent-primary)] transition-colors"
                >
